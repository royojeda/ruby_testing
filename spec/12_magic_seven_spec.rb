...
describe MagicSeven do
  ...
  describe '#play' do
    ...
    context 'when the random number is 9' do
      subject(:nine) { described_class.new(9) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = nine.play
        expect(result).to eq(7)
    ...
    context 'when the random number is 11' do
      subject(:eleven) { described_class.new(11) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = eleven.play
        expect(result).to eq(7)
