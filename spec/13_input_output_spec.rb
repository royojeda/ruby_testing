...
describe NumberGame do
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        first_incorrect_input = 'abc'
        second_incorrecnt_input = '10'
        valid_input = '7'
        allow(game_loop).to receive(:player_input).and_return(first_incorrect_input, second_incorrecnt_input, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game_hard) { described_class.new(5, '5', 7) }
